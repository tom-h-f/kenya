...
# ]
# ///
"""Build Opus v4 partial train/val/challenge splits from prior membership.

    uv run 24_prep_opus_v4.py
    uv run 24_prep_opus_v4.py --force
"""
...
def build_splits(
    df: pd.DataFrame, *, val_size: int, seed: int
) -> tuple[dict[str, pd.DataFrame], dict[str, Any]]:
    if "split" not in df.columns:
        raise ValueError("partial labels are missing prior split membership")
    ...
    unexpected = sorted(set(df["split"]) - {"train", "challenge"})
    if unexpected:
        raise ValueError(f"unexpected prior splits in partial set: {unexpected}")
    ...
    parts = {
        "train2026_opus-v4": shape(train),
        "val2026_opus-v4": shape(val),
        "challenge_opus-v4": shape(challenge),
    }
    report = {
        name: {
            "n": len(frame),
    ...
    report["notes"] = {
        "gold_opus-v4": "unavailable: partial Opus set contains zero prior gold IDs",
...
def main(argv: list[str] | None = None) -> None:
    ...
    parts, report = build_splits(df, val_size=args.val_size, seed=args.seed)

    outputs = {
        name: OUT / f"{name}.parquet"
        for name in parts
    }
