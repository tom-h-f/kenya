"""Export the text-similarity inputs for one snapshot, so the GPU pass can run off-R2.
    ...
    cd analysis && uv run python investigations/2026-09-11-textsim-sensitivity/00_export.py \\
        --snapshot 2026-09-05-promotion-off [--limit 50000] [--texts-only]
...
dedup, the same inner join. Writes `out/<snapshot>[__limitN]/rows.parquet`,
`matrix.npy` and `texts.parquet` (raw and cleaned text, row-aligned with
`rows.parquet`, for re-encoding and word-overlap checks). The matrix is float32
because that is what the GPU pass computes in, so nothing is lost by storing it
that way. `--texts-only` rewrites just `texts.parquet`.
...
def main() -> None:
    ...
    ap.add_argument("--texts-only", action="store_true", help="write texts.parquet and nothing else")
    args = ap.parse_args()

    start = time.perf_counter()
    joined, eligible = export(args.snapshot, args.limit)
    scope = f"__limit{args.limit}" if args.limit else ""
    out = HERE / "out" / f"{args.snapshot}{scope}"
    out.mkdir(parents=True, exist_ok=True)
    joined[["post_id", "text", "clean"]].to_parquet(out / "texts.parquet")
    if not args.texts_only:
        joined[["post_id", "user_id", "created_at"]].to_parquet(out / "rows.parquet")
        matrix = np.vstack(joined["embedding"].to_numpy()).astype(np.float32)
        np.save(out / "matrix.npy", matrix)
        print(f"matrix {matrix.shape}")

    print(f"eligible rows {eligible:,}, with an embedding {len(joined):,}")
