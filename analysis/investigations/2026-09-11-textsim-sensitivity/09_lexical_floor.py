...
`sweep_lex<floor>/edges_t<cut>.parquet`, which `02_score.py` reads.
"""
...
def floored(shard_dir: Path, cut: float, floor: float, clean: np.ndarray, stats: dict):
    cache: dict[int, frozenset] = {}

    def gram_set(row: int) -> frozenset:
        if row not in cache:
            cache[row] = grams(clean[row])
        return cache[row]
...
def main() -> None:
    ...
    edges = coord2.text_similarity_network(
        ...
        similarity=floored(args.shards, args.cut, args.floor, clean, stats),
    )
    out = args.export_dir / f"sweep_lex{args.floor:.2f}"
