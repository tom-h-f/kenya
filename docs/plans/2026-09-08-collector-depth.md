...
## Task 1 run at scale, 2026-09-13/14

3,200 hydration requests over seven bounded passes on pi0: **3,116 hydrated, 84
not found (2.6%), 0 failed**. The terminal-status re-check at scale therefore
passes. 29,171 retweet rows unlocked, at a yield that decayed exactly as the
ledger warns - 45.9, 30.2, 24.5, 17.8, 13.8, 9.4 rows per request as the
densest parents were used up.

**Coverage did not reach 60%, and the target as written is not reachable.**
Retweet rows with a held parent: **222,317 of 445,973, 49.8%**, against 44% on
the 2026-09-05 snapshot. Two reasons the gap is bigger than the plan assumed:
the yield decays faster than the ~3,700-request estimate implied, and the
DENOMINATOR grows - collection continues, so every pass brings new retweets
whose parents are also missing. Reaching 60% from here needs roughly 5,000
further requests at the current yield, and more as it decays.

The honest conclusion is that hydration is not a campaign with an end, it is a
